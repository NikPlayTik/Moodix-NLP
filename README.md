...
Создайте файл `Start_AVG.py` с содержимым, данный скрипт уже имеет внутренний алгоритм присвоения оценки тональности большому по контексту текста (Например: негатив + нейтральное + негативное = негативное):
```python
